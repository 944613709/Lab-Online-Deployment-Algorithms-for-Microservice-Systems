...
def run_single(exp_name, data_dir_path, method):
    ...
    items = [x for x in items if x != '.DS_Store']
    ...
    # TODO 修改了datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    with open(f'./{exp_name}_{method}_{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}.csv', 'w') as f:
        f_csv = csv.writer(f)
...
if __name__ == '__main__':
    ...
    data_path_35000 = 'data/rwt_node_res'
    run_single('experiment1', data_path_35000, 'DFS')
